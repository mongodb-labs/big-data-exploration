...
extends Mapper<Object, BSONObject, Text, BSONWritable> {
	...
	protected double setCounters(double currentPG, BSONObject value, Context context) {
		...
		double distributeDeadEndsPG = ((double) deadEndsPG) / (10E10 * PageRank.totalNodes); 
		...
		// Need this 10E4 because long only takes whole numbers
		context.getCounter(PageRank.RanCounters.RESIDUAL).increment((long) (residual*10E4));
	...
	protected void map(	final Object key, 
		...
		if (links.size() == 0) {
			...
			context.getCounter(PageRank.RanCounters.DEAD_END_PG).
					increment((long) (currentPG * 10E10));
		} else {
